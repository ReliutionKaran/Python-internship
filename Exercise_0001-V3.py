...
class Product(Category):
    ...
    def update_stock_of_products(Products, location, stock_quantity):
        for product in Products:
            ...
            print(f"{product.name} Stock at {L1.name}: {product.stock_at_locations.get(L1, 0)}")
    ...
    def display_products_location(Products, locations):
        for location in locations:
            ...
            location_products = [product for product in Products if location in product.stock_at_locations]
...
class Movement:
    ...
    def move_product(movements):
        for movement in movements:
            ...
            if from_location in product.stock_at_locations and product.stock_at_locations[from_location] >= quantity:
                ...
                if to_location in product.stock_at_locations:
                    ...
                    product.stock_at_locations[to_location] = quantity
                print(f"Moved {quantity} units of {product.name} from {from_location} to {to_location}.")
            else:
                print(f"Error: Insufficient stock of {product.name} at {from_location}.")
